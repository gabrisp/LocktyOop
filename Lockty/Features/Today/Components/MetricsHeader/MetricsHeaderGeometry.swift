...
struct MetricsHeaderGeometry {
    ...
    /// Collapsing thins the ring rather than closing it in on the number: the circle
    /// keeps enough width for the value to sit centred inside it, so what shrinks is the
    /// stroke, not the room.
    static let collapsedDiameter: CGFloat = 46
    ...
    /// Thins right down as it collapses. A hairline around a wide circle is what leaves
    /// the number the middle of it.
    var strokeWidth: CGFloat {
        Self.lerp(7, 1.5, progress: progress)
    }
